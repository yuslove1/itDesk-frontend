...
- **Live task/log board via WebSockets (socket.io)** — the Task Board and Daily Log pages update in real time across every open session, no refresh or polling. `lib/socket.js` holds one shared connection (created lazily, connected on login via `useCurrentUser`, torn down on logout); the socket handshake itself carries the same session cookie as regular API calls, so a socket can't be opened without being logged in. Backend emits `task:created` / `task:updated` / `task:deleted` / `log:created` / `log:updated` / `log:deleted`.

## Live Application URL

**App**: [https://it-desk-frontend.vercel.app](https://it-desk-frontend.vercel.app)

**Backend API**: [https://itdesk-api-final.onrender.com](https://itdesk-api-final.onrender.com)

Both are live and confirmed working together in production — login/logout, `GET /auth/me`, and a real socket.io connection all verified end-to-end against the actual deployed frontend's origin, not just localhost, including the cross-origin `Secure`/`httpOnly` session cookie and CORS accepting the real Vercel domain.
...
- **Live updates**: open `/tasks` as demo.staff in one window and `/manager` (or `/tasks`) as demo.manager in another. Create/move/delete a task in one window and watch it update in the other without a refresh — this is the socket.io connection, not polling. Same test works on `/log` for log entries.
